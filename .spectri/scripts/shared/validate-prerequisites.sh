...
OPTIONS:
  --json              Output in JSON format
  --spec <folder>     Specify spec folder directly (branchless mode)
  --require-tasks     Require tasks.md to exist (for implementation phase)
  --include-tasks     Include tasks.md in AVAILABLE_DOCS list
...
EXAMPLES:
  ...
  # Check task prerequisites for specific spec (branchless)
  ./check-prerequisites.sh --json --spec 011-global-custom-agents
...
if $REQUIRE_TASKS && [[ ! -f "$TASKS" ]]; then
    ...
    echo "Run /spec.tasks first to create the task list."
    exit 1
fi
