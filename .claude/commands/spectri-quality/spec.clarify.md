...
origin:
  ...
  upstream_url: https://github.com/github/spec-kit/blob/main/templates/commands/clarify.md
...
1. Run `.spectri/scripts/shared/validate-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
